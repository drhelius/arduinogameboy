...
void LoadRAMFileInSD()
{
  saveFile = SD.open(ramFileName, FILE_READ);
...
byte PutRAMByte(word address, byte data)
{
  WriteAddress(address);
  PORTF = data;
  PORTL = B00000010;
...
void WriteRAMBank(byte bank)
{
  word maxAddress = 0xC000;
  
  if (mbcType == MBC_2)
  {
    Serial.println("WRITING MBC2 RAM");
    maxAddress = 0xA200;
  }
  else
  {
    Serial.print("WRITING RAM BANK ");
    Serial.println(bank);
  }
  ...
  {
    byte data = saveFile.read();
    PutRAMByte(address, data);
    /*if ((address != 0xA000) && ((address % 16) == 0))
      Serial.println(" ");
    Serial.print(" ");
